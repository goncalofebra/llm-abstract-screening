"""
screener_3txt.py
Versao V1 do screener: 3 ficheiros separados (baseprompt + criteria + abstracts).
Combina os 3 inputs num UNICO user message (estilo Li 2024).

Caracteristicas:
  - Estrutura: [user message] = [Instruction] + [Eligibility Criteria] + [Title] + [Abstract]
  - 1 mensagem chat por chamada (sem system role)
...
Uso:
    python screener_3txt.py --input AbstractTexts/RV3.txt \\
                            --base baseprompt.txt \\
                            --criteria eligibilitycriteria_SR.txt \\
                            --tag RV3_v1
...
NOTA: para a versao V2 (system+user, 2 ficheiros), usa screener_2txt.py.
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        prog="screener_3txt",
        description="V1: 3 ficheiros (baseprompt + criteria + abstract) -> 1 user message",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument("--input", required=True, help="AbstractTexts/<tag>.txt")
    p.add_argument("--base", required=True, help="Instrucao generica (ex: prompts/baseprompt_RV3.txt)")
    p.add_argument("--criteria", required=True, help="eligibilitycriteria_*.txt da SR/MA")
    p.add_argument("--tag", required=True, help="Identificador (ex: RV3_v1, RV3_v1_qwen3)")
...
def main():
    ...
    print(f"[info] V1 (single message)  provider={args.provider}  model={model}")
    ...
    for idx, rec in enumerate(records):
        ...
        # V1: tudo num so user message.
        # Ordem: [Instruction] + [Eligibility Criteria] + [Title] + [Abstract]
        full_prompt = (
            f"{base_prompt}\n\n"
    ...
    print(f"\nV1 feito em {time.time()-t0:.1f}s - {total_tokens} tokens - outputs em {outdir}/")
